...
panel.innerHTML = `
  <div class="fp-header">
    ...
    <div class="fp-header-right">
      ...
      <button class="fp-refresh-btn" title="刷新">&#8635;</button>
      <button class="fp-close-btn" title="关闭">&times;</button>
    </div>
  </div>
  <div class="fp-body">
...
function renderFileList() {
  ...
  for (const f of sorted) {
    ...
    item.innerHTML = `
      ...
      <a class="fp-file-download" href="${escapeHtml(state.conversationId ? fileUrl(state.conversationId, f.path) : '#')}" download="${escapeHtml(f.name)}" target="_blank" title="下载">&#8595;</a>
    `;
...
async function openFile(f) {
  ...
  previewMeta.innerHTML = `
    ...
    <a class="fp-meta-download" href="${escapeHtml(dlUrl)}" download="${escapeHtml(currentFile.name)}" target="_blank" title="下载">&#8595; 下载</a>
  `;
