...
- `src/agent_cli/renderer.py` — Claude Code-style event renderer. `StreamingRenderer` class buffers streaming tokens and syntax-highlights completed fenced code blocks via `rich.Syntax`. Thinking spinner via `rich.Status`. Enhanced done line with cost. Handles all event types: text_delta, thinking_delta, thinking, tool_call, tool_result, tool_approval, subagent (parallel: tracks `_subagent_count`, shows agent type label, spinner displays "Researching (N subagents)" when multiple active), todo, task, compact, background, teammate, plan_mode_changed, teams_changed, approval_changed, plan_ready, plan_approved, plan_rejected, interrupted, error, done
