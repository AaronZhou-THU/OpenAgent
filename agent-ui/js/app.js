...
function setStreamingUI(streaming) {
  ...
  if (streaming) {
    input.placeholder = 'Type to interrupt the agent...';
  ...
  switch (event.type) {
    ...
    case 'background_result':
      ...
      for (const n of (event.notifications || [])) {
        appendCompactNotice(`Background [${n.task_id}] ${n.status}: ${(n.result || '').slice(0, 200)}`);
      }
      break;

    case 'teammate_status':
      appendCompactNotice(`Teammate "${event.name}" (${event.role || '?'}): ${event.status}`);
      break;
    ...
    case 'teams_changed':
      ...
      updateTeamsUI();
      appendCompactNotice(event.enabled ? 'Teams enabled' : 'Teams disabled');
      break;

    case 'approval_changed':
      state.approvalActive = event.enabled;
      updateApprovalUI();
      appendCompactNotice(event.enabled ? 'Tool approval enabled' : 'Tool approval disabled');
      break;

    case 'plan_mode_changed':
      state.planModeActive = event.enabled;
      updatePlanModeUI();
      appendCompactNotice(event.enabled ? 'Plan mode enabled' : 'Plan mode disabled');
      break;
    ...
    case 'plan_approved':
      ...
      updatePlanModeUI();
      appendCompactNotice('Plan approved — switching to execution mode');
      break;

    case 'plan_rejected':
      hidePlanApproval();
      if (event.feedback) {
        appendCompactNotice(`Plan rejected: ${event.feedback}`);
      }
      break;
...
function updateTeamsUI() {
  ...
  if (state.teamsActive) {
    btn.classList.add('active');
    btn.title = 'Teams ON (click to disable)';
  } else {
    btn.classList.remove('active');
    btn.title = 'Teams OFF (click to enable)';
  }
}
...
function updateApprovalUI() {
  ...
  if (state.approvalActive) {
    btn.classList.add('active');
    btn.title = 'Approval ON (click to disable)';
  } else {
    btn.classList.remove('active');
    btn.title = 'Approval OFF (click to enable)';
  }
}
