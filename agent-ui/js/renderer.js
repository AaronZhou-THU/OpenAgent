  ...
  for (const conv of state.conversations) {
    ...
    item.innerHTML = `
      <div class="conv-info">
        ...
        <div class="conv-meta">${date} &middot; ${count} msgs</div>
      </div>
...
export function renderHistory(conversation) {
  ...
  document.getElementById('chat-title').textContent = conversation.title || 'Untitled';
...
export function appendSubagentStart(task, agentType) {
  ...
  block.setAttribute('aria-label', `Subagent working on: ${task}`);
...
export function appendSubagentEnd(blockId, summary, toolCount, elapsed, usage) {
  ...
  statsEl.textContent = `${toolCount ?? 0} tool calls \u00b7 ${elapsedStr}${tokensStr ? ' \u00b7 ' + tokensStr : ''}`;
...
export function appendToolApprovalRequest(tools) {
  ...
  block.innerHTML = `
    ...
    <div class="approval-actions" id="${blockId}-actions">
      <button class="approval-btn approve-btn" data-decision="approve">Approve</button>
      <button class="approval-btn deny-btn" data-decision="deny">Deny</button>
      <button class="approval-btn auto-approve-btn" data-decision="auto_approve">Auto-approve rest</button>
    </div>
  `;
...
export function renderTodos(todos) {
  ...
  if (!todos || todos.length === 0) {
    listEl.innerHTML = '<p style="color: var(--text-muted); font-size: 13px;">No tasks yet.</p>';
    return;
  }
