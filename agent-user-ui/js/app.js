...
function bindUI() {
  ...
  document.getElementById('plan-change-submit').addEventListener('click', () => {
    ...
    hidePlanOverlay();
    finishAssistantMessage();
    renderUserMessage(feedback || 'Revise the plan');
    state.pendingPlan = null;
    startAssistantMessage();
  });
}
...
async function deleteConversation(id) {
  try {
    ...
    if (state.conversationId === id) {
      ...
      document.getElementById('chat-title').textContent = 'Assistant';
...
function setStreamingUI(streaming) {
  ...
  if (streaming) {
    input.placeholder = 'Type to interrupt...';
...
function handleServerEvent(event) {
  switch (event.type) {
    ...
    case 'subagent_start':
      activeSubagentCount++;
      finalizeTextBlock();
      showActivity(activeSubagentCount > 1 ? `Working on ${activeSubagentCount} tasks...` : 'Researching...');
      break;
