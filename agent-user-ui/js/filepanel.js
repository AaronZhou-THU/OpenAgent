// File Panel — workspace file browser (Forest Canopy edition)
...
panel.innerHTML = `
  <div class="fp-header">
    <div class="fp-header-left">
      <button class="fp-back-btn" style="display:none" title="Back to file list">&#8592;</button>
      <span class="fp-title">Files</span>
    </div>
    <div class="fp-header-right">
      <button class="fp-upload-btn" title="Upload files">&#8593;</button>
      <button class="fp-refresh-btn" title="Refresh">&#8635;</button>
...
toggleBtn.title = 'Browse workspace files';
  ...
  for (const f of sorted) {
    ...
    item.innerHTML = `
      ...
      <div class="fp-file-info">
        <span class="fp-file-name" title="${escapeHtml(f.path)}">${escapeHtml(f.path)}</span>
        <span class="fp-file-size">${formatSize(f.size)}</span>
      </div>
      <a class="fp-file-download" href="${escapeHtml(state.conversationId ? fileUrl(state.conversationId, f.path) : '#')}" download="${escapeHtml(f.name)}" target="_blank" title="Download">&#8595;</a>
    `;
    // Click row to preview, click download icon to download
    item.querySelector('.fp-file-info').addEventListener('click', () => openFile(f));
    item.querySelector('.fp-file-icon').addEventListener('click', () => openFile(f));
...
async function openFile(f) {
  ...
  previewContent.innerHTML = '<div class="fp-loading">Loading...</div>';
  previewMeta.innerHTML = '';

  try {
    currentFile = await readWorkspaceFile(f.path);
  } catch (err) {
    previewContent.innerHTML = '<div class="fp-error">Failed to load file</div>';
    return;
  }
  ...
  previewMeta.innerHTML = `
    ...
    <a class="fp-meta-download" href="${escapeHtml(dlUrl)}" download="${escapeHtml(currentFile.name)}" target="_blank" title="Download">&#8595; Download</a>
  `;

  if (currentFile.binary) {
    previewContent.innerHTML = '<div class="fp-binary-notice">Binary file &mdash; preview not available</div>';
    return;
  }

  if (currentFile.content === null || currentFile.content === undefined) {
    previewContent.innerHTML = '<div class="fp-binary-notice">No content available</div>';
    return;
  }
  ...
  if (lang === 'markdown' || ext === 'md') {
    ...
    previewContent.innerHTML = `<div class="fp-markdown assistant-content">${html}</div>`;
    return;
  }
